...
SELECT FIRST_NAME || ' ' || LAST_NAME AS NAME, PHONE_NUMBER ,DEPARTMENT_ID FROM EMPLOYEES WHERE DEPARTMENT_ID IN(50);

--5. 모든 사원의 이름, 급여 그리고 300달러 인상된 급여를 출력하세요.
SELECT FIRST_NAME || ' ' || LAST_NAME AS NAME, SALARY + 300 AS 인상된급여 FROM EMPLOYEES;
...
SELECT FIRST_NAME || ' ' || LAST_NAME AS NAME, JOB_ID, COMMISSION_PCT FROM EMPLOYEES;
...
SELECT FIRST_NAME || ' ' || LAST_NAME AS NAME, SALARY FROM EMPLOYEES WHERE DEPARTMENT_ID IN(60) ORDER BY SALARY DESC;
...
SELECT FIRST_NAME || ' ' || LAST_NAME AS NAME FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR JOB_ID = 'SA_MAN';

--13. Steven King 사원의 정보를 “Steven King 사원의 급여는 24000달러 입니다” 형식으로 출력하세요.
SELECT FIRST_NAME || ' ' || LAST_NAME||  ' 사원의 급여는 ' || SALARY || ' 입니다' AS GOOD FROM EMPLOYEES
